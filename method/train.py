...
def evaluate_full_batch(model, minibatch, mode='val'):
    """
    Full-batch evaluation: only for validation and test set.
    When calculating F1 score, we mask out the relevant root nodes
    (e.g., those belonging to val/test sets).
    
    Args:
        model: model to be evaluated
        minibatch: minibatch data loader
        mode: 'val', 'test' or 'valtest'
    
    Returns:
        loss: loss value
        f1mic: micro F1 score
        f1mac: macro F1 score
    """
    ...
    # loss is not very accurate in this case, since loss is also contributed from training nodes.
    # on the other hand, for val / test, we only care about their accuracy.
    # so the loss issue is not a problem.
    return loss, f1mic, f1mac

def prepare(train_data,train_params,arch_gcn):
    """
    Prepare data structures and initialize model / minibatch(sampler) before an actual iterative training.
    Args:
        train_data:
        train_params:
        arch_gcn:
...
def train(train_phases, model, minibatch, minibatch_eval, model_eval, eval_val_every):
    """
    Perform model training, including training, validation and testing.
    Args:
        train_phases: configuration for training phases
        model: model for training
        minibatch: minibatch loader for training
        minibatch_eval: minibatch loader for evaluation
        model_eval: model for evaluation
        eval_val_every: evaluate validation set every this number of epochs
    """
